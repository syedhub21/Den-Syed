...
export async function setSessionCookie(token: string) {
  ...
  store.set(COOKIE_NAME, token, {
    httpOnly: true,
    // SameSite=None lets the session cookie round-trip when the site is
    // viewed inside a cross-origin iframe (e.g. an embedded preview panel),
    // where browsers otherwise block Lax cookies as third-party. None
    // requires Secure — fine on Vercel (HTTPS) and on localhost (modern
    // browsers treat localhost as a secure context). In production the
    // site is first-party, so Lax is the stricter, safer default.

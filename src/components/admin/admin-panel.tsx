...
import { X, Lock, LogOut, Loader2, Plus, Trash2, Save, Mail, Upload } from "lucide-react";
...
type Tab = "profile" | "projects" | "services" | "techstack" | "stats" | "messages";
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="admin-panel fixed inset-0 z-[200] bg-bg/95 backdrop-blur-sm overflow-y-auto"
          role="dialog"
          aria-modal="true"
              ...
              {checking ? (
                ...
                <div className="flex flex-col md:flex-row min-h-[500px]">
                  ...
                  <div className="flex-1 p-4 md:p-6 overflow-y-auto max-h-[70vh]">
                    ...
                    {tab === "stats" && <StatsEditor onChanged={onDataChanged} />}
...
const MAX_UPLOAD_BYTES = 2 * 1024 * 1024; // 2MB — matches /api/upload

/**
 * ImageField — upload-first image picker for the admin panel.
 * Upload button + live thumbnail preview; the path/URL input below stays
 * editable for advanced use (external URLs still work).
 */
function ImageField({
  label,
  value,
      ...
      <div className="flex items-center gap-2">
        {value && (
          <img
            src={value}
            alt=""
            className="w-12 h-12 rounded-lg object-cover border border-stroke flex-shrink-0 bg-bg"
          />
        )}
        <button
          type="button"
          ...
          disabled={uploading}
          className="inline-flex items-center gap-1.5 text-xs rounded-xl border border-stroke text-text-primary px-3 py-2 font-body font-medium hover:border-accent/50 hover:text-accent transition-colors disabled:opacity-50"
        >
          {uploading ? (
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData({
        ...
        nowWorkingOn: row.nowWorkingOn ?? "",
        nowCurrently: row.nowCurrently ?? "",
        nowReading: row.nowReading ?? "",
        quoteText: row.quoteText || "If it doesn't ship, it doesn't exist.",
        quoteAttribution: row.quoteAttribution || "house rule",
        email: row.email ?? "",
        phone: row.phone ?? "",
      ...
      setError("Couldn’t load the profile. Check your connection and retry.");
    }
  }, []);
      ...
      <div className="py-10 text-center space-y-3">
        {error ? (
          <>
            ...
            <button
              onClick={load}
              className="inline-flex items-center gap-2 text-xs rounded-xl bg-accent text-bg px-4 py-2 font-body font-medium"
            >
              Retry
            </button>
          </>
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-[10px] text-muted/70 font-body">
        The stat number cards live in the <strong>Stats</strong> tab and the
        “What I do” list in the <strong>Services</strong> tab.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <ImageField
          label="Hero photo (portrait card)"
          value={data.heroImage}
          onChange={(v) => setData({ ...data, heroImage: v })}
        />
        <ImageField
          label="About photo (studio card)"
          value={data.aboutImage}
          onChange={(v) => setData({ ...data, aboutImage: v })}
        />
      </div>
      <Field label="Bio — second manifesto paragraph" value={data.bio} onChange={(v) => setData({ ...data, bio: v })} textarea rows={5} />

      <div className="pt-2 border-t border-stroke">
        <h4 className="text-xs uppercase tracking-[0.2em] text-muted font-body mb-1">
          Manifesto — opening paragraph
        </h4>
        <p className="text-[10px] text-muted/70 font-body mb-3">
          The big serif opener in the About bento. Wrap words in *asterisks* to
          paint them orange-italic.
        </p>
        <Field
          label="Manifesto"
          value={data.manifesto}
          onChange={(v) => setData({ ...data, manifesto: v })}
          textarea
          rows={4}
        />
      </div>
      ...
      <div className="pt-2 border-t border-stroke">
        <h4 className="text-xs uppercase tracking-[0.2em] text-muted font-body mb-1">
          Now — “What I’m up to” section
        </h4>
        <p className="text-[10px] text-muted/70 font-body mb-3">
          Wrap words in *asterisks* for the green highlight. The fourth cell
          uses your Email from the fields above.
        </p>
        <div className="space-y-4">
          <Field
            label="Working on"
            value={data.nowWorkingOn}
            onChange={(v) => setData({ ...data, nowWorkingOn: v })}
            textarea
            rows={2}
          />
          <Field
            label="Currently"
            value={data.nowCurrently}
            onChange={(v) => setData({ ...data, nowCurrently: v })}
            textarea
            rows={2}
          />
          <Field
            label="Reading"
            value={data.nowReading}
            onChange={(v) => setData({ ...data, nowReading: v })}
            textarea
            rows={2}
          />
        </div>
      ...
      <div className="pt-2 border-t border-stroke">
        <h4 className="text-xs uppercase tracking-[0.2em] text-muted font-body mb-1">
          Quote — the orange card
        </h4>
        <p className="text-[10px] text-muted/70 font-body mb-3">
          The big quote in the About bento (defaults to “If it doesn’t ship, it
          doesn’t exist. — house rule”).
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field
            label="Quote text"
          ...
          <Field
            ...
            onChange={(v) => setData({ ...data, quoteAttribution: v })}
          />
        </div>
      </div>
    </div>
...
function ProjectForm({
  ...
  return (
    <div className="space-y-4">
      ...
      <ImageField
        label="Cover image — upload a file"
...
function StatsEditor({ onChanged }: { onChanged: () => void }) {
  ...
  const saveOne = async (s: Stat) => {
    setSaving(s.id);
    try {
      const res = await fetch(`/api/admin/stats/${s.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(s),
      });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-display text-2xl text-text-primary">Stats</h3>
        <button
          onClick={add}
          className="inline-flex items-center gap-1.5 text-xs rounded-xl bg-accent text-bg px-3 py-1.5 font-body font-medium"
        >
      ...
      <p className="text-[10px] text-muted/70 font-body">
        The numbered cards in the About bento — e.g. Projects Completed 20+,
        Years Experience 2+. The first two show by default; add more and the
        grid adapts.
      </p>
      <div className="space-y-2">
        {items.map((s) => (
          <div
            key={s.id}
            className="grid grid-cols-12 gap-2 items-end p-3 bg-bg border border-stroke rounded-xl"
          >
            <div className="col-span-5">
              <Field
                label="Label"
            ...
            <div className="col-span-3">
              <Field
                ...
                onChange={(v) => update(s.id, { value: Number(v) || 0 })}
              />
            </div>
            <div className="col-span-2">
              <Field
                label="Suffix"
                value={s.suffix}
                onChange={(v) => update(s.id, { suffix: v })}
              />
            </div>
            <div className="col-span-2 flex items-center gap-1">
              <button
                ...
                className="text-xs text-text-primary hover:opacity-80 px-2 py-2 font-body"
              >
                {saving === s.id ? "…" : "Save"}
              </button>
              <button
